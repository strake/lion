...
byteMaskX yl@SNat addr =
    foldr (.|.) 0 ((\ k -> byteMask8 (addr + (resize . pack) k)) <$> indices (divSNat yl d8))

byteMask8 :: (KnownNat xl, 1 <= Div xl 8) => BitVector xl -> BitVector (Div xl 8)
byteMask8 =
    (1 `shiftL`) . unpack . resize .
    (resizeTo =<< flogBaseSNat d2 . flip divSNat d8 . snatProxy)

sliceX :: forall xl yl
  .  (KnownNat xl, 1 <= yl)
  => SNat yl -> BitVector (Div xl 8) -> BitVector xl -> BitVector yl
sliceX yl@SNat mask = resizeTo yl . foldl fromMaybe (pure 0) (imap f mask')
  where
